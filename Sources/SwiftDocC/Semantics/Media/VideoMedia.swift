...
extension VideoMedia: RenderableDirectiveConvertible {
    func render(with contentCompiler: inout RenderContentCompiler) -> [RenderContent] {
        ...
        if let poster = poster {
            let unescapedPosterSource = poster.path.removingPercentEncoding ?? poster.path
            let posterIdentifier = RenderReferenceIdentifier(unescapedPosterSource)
            if let resolvedPosters = contentCompiler.context.resolveAsset(
                named: unescapedPosterSource,
                in: contentCompiler.identifier
            ) {
                contentCompiler.imageReferences[unescapedPosterSource] = ImageReference(
                    identifier: posterIdentifier,
                    imageAsset: resolvedPosters
                )
            }
            
            posterReferenceIdentifier = posterIdentifier
        }
