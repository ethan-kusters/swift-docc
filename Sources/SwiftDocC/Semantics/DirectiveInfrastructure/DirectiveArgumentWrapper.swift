...
@propertyWrapper
public struct _DirectiveArgument<Value>: _DirectiveArgumentProtocol {
    ...
    func setProperty<T>(
        ...
        let path = T.keyPaths[propertyName] as! ReferenceWritableKeyPath<T, _DirectiveArgument<Value>>
        let wrappedValuePath = path.appending(path: \Self.parsedValue)
        containingDirective[keyPath: wrappedValuePath] = any as! Value?
    }
}

extension _DirectiveArgument where Value: DirectiveArgumentValueConvertible {
...
extension _DirectiveArgument where Value: OptionallyWrappedDirectiveArgumentValueConvertible {
    init(
        wrappedValue: Value,
        name: _DirectiveArgumentName = .inferredFromPropertyName,
        required: Bool = false
