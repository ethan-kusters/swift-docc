...
extension AutomaticDirectiveConvertible {
    ...
    func parseChildMarkup(
        ...
        fatalError("""
            'parseChildMarkup' is required for automatic directive convertibles that \
            contain multiple '@ChildMarkup' properties.
            """
        )
    }
}
    ...
    public init?(
            ...
            let firstChildMarkup = markupRequirements.first
        {
            if markupRequirements.count < 2 {
                ...
                let successfullyParsedChildMarkup = parseChildMarkup(
                    from: remainder,
                    source: source,
                    for: bundle,
                    in: context,
                ...
                if !successfullyParsedChildMarkup {
                    unableToCreateParentDirective = true
                }
            }
